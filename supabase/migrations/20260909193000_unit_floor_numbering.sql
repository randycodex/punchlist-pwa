-- Support the optional project unit-floor convention, preserving checkpoint rules.
...
begin
  ...
  if exists (
    ...
    where payload_key.key <> all (array[
      'projectName', 'address', 'date', 'inspector', 'gcName', 'gcSignoff',
      'facadeLevelStart', 'facadeLevelEnd', 'checkpointRules', 'unitFloorNumbering'
    ])
  ) then
    raise exception 'Shared project metadata payload contains unsupported fields.' using errcode = '22023';
  end if;

  if p_metadata_payload ? 'unitFloorNumbering' and (
    jsonb_typeof(p_metadata_payload -> 'unitFloorNumbering') <> 'string'
    or p_metadata_payload ->> 'unitFloorNumbering' not in ('prefix', 'last-two-digits', 'manual')
  ) then
    raise exception 'Invalid unit floor numbering convention.' using errcode = '22023';
  end if;
  ...
  if not (p_metadata_payload ? 'unitFloorNumbering') then
    p_metadata_payload := p_metadata_payload || coalesce(
      (select jsonb_build_object('unitFloorNumbering', metadata_payload -> 'unitFloorNumbering')
       from public.shared_project_metadata_snapshots where project_id = p_project_id
         and metadata_payload ? 'unitFloorNumbering'), '{}'::jsonb);
  end if;
